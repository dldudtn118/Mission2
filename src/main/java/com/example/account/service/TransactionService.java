...
public class TransactionService {
	...
	/**
	 * 사용자 없는 경우, 계좌가 없는경우, 사용자 아이디와 계좌 소유주가 다른 경우,
	...
	public TransactionDto useBalance(Long userId, String accountNumber, Long amount) {
		...
		Transaction transaction = saveAndGetTransaction(TransactionType.USE, TransactionResultType.S
									,account,amount);
	...
	private void validateCancelBalance(Transaction transaction, Account account, Long amount) {
		...
		if(!Objects.equals(transaction.getAmount(),amount)) {//Long끼리는 equals사용해야함. 128부터 ==사용불가능 
	...
	@Transactional
	public void saveFailedCancelTransaction(String accountNumber, @NotNull Long amount) {
		Account account = accountRepository.findByAccountNumber(accountNumber)
				.orElseThrow(() -> new AccountException(ErrorCode.ACCOUNT_NOT_FOUNT));
